...
(() => {
  ...
  const MESSENGER_SYNC_API_KEY = '__SHIPIDLE_MESSENGER_SYNC__';
  ...
  const MESSENGER_TABLE = 'messenger_sync';
  ...
  function validateMessengerRoomKey(roomKey) {
    const value = String(roomKey || '');
    if (!/^\/(stories\/[^/]+\/episodes|characters\/[^/]+\/chats|u\/[^/]+\/c)\/[^/?#]+$/.test(value)) {
      throw new Error('메신저 설정 채팅방 경로가 올바르지 않음.');
    }
    return value;
  }
  ...
  function normalizeMessengerSettings(settings) {
    ...
    return {
      ...
      model,
      contextTurns,
      characterAvatar: normalizeMessengerAvatar(settings?.characterAvatar),
      userAvatar: normalizeMessengerAvatar(settings?.userAvatar),
      characterCrop: normalizeMessengerCrop(settings?.characterCrop),
      userCrop: normalizeMessengerCrop(settings?.userCrop),
    };
  }

  function normalizeMessengerAvatar(value) {
    const avatar = String(value || '');
    return avatar.length <= 700000 && /^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(avatar) ? avatar : '';
  }

  function normalizeMessengerCrop(value) {
  ...
  async function getMessengerSettings(roomKey) {
    const active = await refreshSessionIfNeeded();
    const path = validateMessengerRoomKey(roomKey);
    const query = `owner_id=eq.${encodeURIComponent(active.user.id)}&room_key=eq.${encodeURIComponent(path)}&select=settings,revision,updated_at,device_label`;
    const rows = await authedRequest(`${config.projectUrl}/rest/v1/${MESSENGER_TABLE}?${query}`);
    return rows?.[0] || null;
  }

  async function saveMessengerSettings({ roomKey, settings, revision, deviceLabel } = {}) {
    const active = await refreshSessionIfNeeded();
    const path = validateMessengerRoomKey(roomKey);
    const safeRevision = Math.max(1, Number(revision) || 1);
    const safeSettings = normalizeMessengerSettings(settings);
    const rows = await authedRequest(`${config.projectUrl}/rest/v1/${MESSENGER_TABLE}?on_conflict=owner_id,room_key`, {
      method: 'POST',
  ...
  Object.defineProperty(BRIDGE, MESSENGER_SYNC_API_KEY, {
    configurable: true,
    value: Object.freeze({
      version: 1,
      getStatus: messengerSyncStatus,
      getSettings: getMessengerSettings,
      saveSettings: saveMessengerSettings,
    }),
  });
