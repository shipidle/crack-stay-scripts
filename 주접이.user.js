...
(() => {
  ...
  function promptFor(store, payload) {
    ...
    return [
      ...
      '항상 한국어 1문장. 줄바꿈 금지. 40~70자 안에서 완결된 웃긴 주접을 쓴다. 문장을 중간에 끊지 말고 반드시 끝맺는다. 마지막에는 장면에 어울리는 카오모지 하나를 네가 직접 골라 붙인다.',
  ...
  function talkPromptFor(store, text) {
    ...
    return [
      ...
      '항상 한국어 1문장. 줄바꿈 금지. 40~70자 안에서 완결된 웃긴 반응을 쓴다. 마지막에는 장면에 어울리는 카오모지 하나를 붙인다.',
 ...
 function cleanOneSentence(text) {
  ...
  function fallbackLine(text) {
    const samples = [
      '머임? 저 눈빛이면 주민등록상 연애 중이어야 됨 (˶ᵔᵕᵔ˶)',
      '아니 저러고 안 사귄다고? 나 오늘도 상식 패배함 (•⤙•)',
      '둘이 감정선으로 줄다리기하다가 전봇대 뽑겠는데 ( ˙꒳˙ )',
      '냐냐냥!!! 냐냐냥!!! 냐냐냥!!! ฅ(ΦωΦ)ฅ',
      '저 분위기면 국세청도 썸세 걷으러 옴 (◜◡◝)',
      '말은 아닌 척하는데 심장은 이미 혼인신고서 출력함 ( ᐛ )',
    ];
    const spice = TENDENCIES.spice.words.test(text) ? '야르방댕이 공개쇼 직전의 공기 뭐임 ( ´ཀ` )' : '';
  ...
  function renderPanel(view = 'log') {
    ...
    panel.querySelector('[data-action="talk"]')?.addEventListener('click', async () => {
      ...
      try {
        ...
        const line = 'API가 또 드러누움. 이건 설정이 아니라 서버 컨디션 문제 같음 ( ᐛ )';
  ...
  function savePanel(announce = true) {
    ...
    if (announce) say('설정 저장함. 이제 주접 대기 탄다 ( ᐛ )', 3200);
  }
  ...
  function scheduleScan(delay = 700) {
   ...
   async function scanLatest() {
    ...
    try {
      ...
      const line = 'API가 삐끗함. 그래도 이 장면 냄새는 수상함 ( ᐛ )';
