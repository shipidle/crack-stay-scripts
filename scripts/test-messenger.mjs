...
assert.match(messenger, /const VERSION = '0\.4\.1'/);
...
assert.match(messenger, /const maxSize = 768/);
assert.match(messenger, /while \(encoded\.length > 680000/);
assert.match(messenger, /left:\$\{left\.toFixed\(2\)\}%/);
assert.doesNotMatch(messenger.match(/function avatarMarkup\([\s\S]*?\n  \}/)?.[0] || '', /transform:scale/);
assert.match(messenger, /name="cms-message-composer" autocomplete="off"/);
assert.match(messenger, /name="cms-character-habits" autocomplete="off"/);
assert.match(messenger, /composerInput\.defaultValue = ''/);
