...
assert.match(messenger, /'gemini-3\.5-flash-lite': \{ label: 'Gemini 3\.5 Flash-Lite · 초절약', input: 0\.30, output: 2\.50/);
assert.match(messenger, /'gemini-3\.6-flash': \{ label: 'Gemini 3\.6 Flash · 추천', input: 1\.50, output: 7\.50/);
assert.match(messenger, /'gemini-3\.1-pro-preview': \{ label: 'Gemini 3\.1 Pro Preview', input: 2\.00, output: 12\.00/);
assert.match(messenger, /'gemini-2\.5-pro': 'gemini-3\.6-flash'/);
...
assert.match(messenger, /const VERSION = '0\.4\.0'/);
assert.match(messenger, /#cms-status\{[^}]*white-space:normal/);
